...
tokens:
  ...
  colors:
    ...
    on-primary: "#000000"
  typography:
    family: { sans: "Noto Sans KR", mono: "SF Mono" }
    display-hero:  { size: 36, weight: 800, lineHeight: 1.25, use: "Splash screens, landing" }
    display-large: { size: 28, weight: 700, lineHeight: 1.30, use: "Service section titles" }
...
TokCatch (톡캐치) is a quiet tool that stays in the background. The user — a PM juggling several projects — is the protagonist; the app's job is to take whatever they toss in and set it in its place without fuss. So the interface is a clean, functional canvas that never competes for attention, lit by a single warm accent (`#FEE500`) reserved for the few moments that matter: the throw, the match, the confirmation. The yellow isn't decoration here — it's a quiet signal that something landed safely, used sparingly against calm neutrals.

The design philosophy is "던지면 제자리에" (toss it, and it lands where it belongs). Every decision serves capture-then-calm: the moment of throwing an idea should feel weightless, and the moment of reviewing should feel orderly. The interface should be invisible enough that a half-formed keyword goes in without friction, yet legible enough that a glance later tells you exactly where it went. Capture cards and project chips are the defining surfaces — plain, scannable, never shouty.

Typography is deliberately neutral and Korean-first — Korean microcopy lives on every screen, so legible Hangul at small sizes matters more than a branded display face. The overall aesthetic is flat, calm, and content-forward: minimal shadows, no gradients, hierarchy carried by background-color layering and a single restrained accent rather than elevation or color noise.

**Key Characteristics:**
- Signal Yellow (`#FEE500`) as the singular accent -- used sparingly, only for capture/match/confirm moments, never as decoration
- Korean-first neutral type so half-formed keywords and quick scans read effortlessly
- Calm, content-forward surfaces -- capture cards and project chips, not shouty UI
- Flat design with minimal shadow -- depth through background color layering, not elevation
- Near-black (`#1E1E1E`) base instead of pure black -- subtle warmth, never cold
- 12px border-radius as the universal standard for interactive elements
- Restraint over emphasis: the voice never raises its hand, and neither does the UI

## 2. Color Palette & Roles

### Primary
- **Signal Yellow** (`#FEE500`): The single accent. Primary CTA ([던지기]), the "유력" recommendation chip, success confirmation. Used sparingly — it marks moments, not surfaces.
- **Near Black** (`#1E1E1E`): Base color. Wordmark, primary text. Warm, never pure black.
- **Pure White** (`#ffffff`): App background, card surfaces.

### Accent Variants
- **Catch Highlight** (`#FEE500`): Fills the recommended ("유력") project chip and the throw CTA — the "it landed" signal.
- **Surface White** (`#ffffff`): Capture cards, list rows, with a subtle `#E5E5E5` border.
- **Divider Fill** (`#F0F0F0`): Section dividers, toast backgrounds, empty-state fills.
- **Attention Yellow** (`#FAEB00`): Reserved for the rare count/marker that must draw the eye.
...
### Font Family
- **UI Primary**: `"Noto Sans KR", -apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", Roboto, "Malgun Gothic", sans-serif`
- **Monospace**: `"SF Mono", SFMono-Regular, Menlo, Consolas, monospace`

Noto Sans KR (Google-hosted, OFL-1.1) replaces the reference's system-font + brand-display split: it covers the full Hangul glyph set with multiple weights and renders consistently across devices, which matters because Korean microcopy is on every screen. Display sizes (36/28px) use the same family at heavier weights rather than a separate branded face — restraint over a "brand voice" typeface.

### Hierarchy

| Role | Font | Size | Weight | Line Height | Letter Spacing | Notes |
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | Noto Sans KR | 36px | 800 | 1.25 | normal | Welcome / landing |
| Display Large | Noto Sans KR | 28px | 700 | 1.30 | normal | Section titles |
| Heading Large | Noto Sans KR | 22px | 700 | 1.36 | normal | Screen titles, major sections |
| Heading | Noto Sans KR | 20px | 600 | 1.40 | normal | Navigation titles, modal headers |
| Title | Noto Sans KR | 18px | 600 | 1.44 | normal | Project names, card titles |
| Body | Noto Sans KR | 16px | 400 | 1.50 | normal | Memo text, descriptions |
| Body Small | Noto Sans KR | 14px | 400 | 1.57 | normal | Secondary info, metadata |
| Caption | Noto Sans KR | 13px | 400 | 1.54 | normal | Timestamps, status text |
| Caption Small | Noto Sans KR | 12px | 400 | 1.50 | normal | Fine print, badges |
| Micro | Noto Sans KR | 11px | 400 | 1.45 | normal | Tab bar text, smallest labels |

### Principles
- **One neutral family**: A single Korean-first sans across the whole product. A half-formed keyword the user just tossed in should look like their note, not a designed artifact — no branded display face competing with their words.
- **Hangul legibility first**: Korean microcopy is on every screen, often at small sizes. The family is chosen so 12–14px Hangul stays clean on mid-range devices.
- **Weight restraint**: Most UI uses 400-500 weight. Bold (700) only for project names, screen titles, counts. The voice never raises its hand — neither does the type. Calm over emphasis competition.

## 4. Component Stylings

### Buttons

The product uses one accent yellow: `#FEE500` for the primary throw CTA ([던지기], [+ 아이디어 던지기]). A slightly deeper `#fae100` and bordered pills are kept for the landing/intro page only. The yellow CTA is the single loud element on any screen — everything else stays neutral.
...
- Use: The throw action ([던지기]) and other primary confirmations. One per screen — the accent loses meaning if repeated.
...
- Padding: 7px 13px
- Font: 13px / 700
- Use: Landing/intro page CTA only — bordered yellow pill, the one place a louder marketing shape is allowed
...
- Padding: 7px 8px
- Font: 13px / 700
- Use: Landing CTA paired with the yellow Marketing Pill (secondary action)
...
- Padding: 3px 14px
- Font: 16px / 700
- Use: Active item in a top nav/filter row — full-pill rounded, heavier display weight
...
- Padding: 10px 20px
- Font: 12px / 400
- Use: Footer link pill on the landing/intro page
...
- Use: Form fields (new project name, login)
...
- Font: 16px / 400
- Placeholder: `#BBBBBB`
- Use: The throw composer — auto-focused, placeholder "여기에 툭 던지세요"
...
- Font: 14px / 400
- Placeholder: `#999999`
- Use: Search bar (memos, projects) — left-icon at `#999999`
...
- Use: Most surfaces (capture cards, list rows) — intentionally flat, shadows are barely-there
...
- Padding: 8px 12px
- Font: 14px / 400
- Use: Yellow-accented inline marker, borrowed for the rare warm highlight (e.g. an echo of a just-tossed memo). Asymmetric radius.
...
- Padding: 8px 12px
- Font: 14px / 400
- Use: Neutral inline content block on white surfaces.
...
- Padding: 4px 12px
- Font: 12px / 400
- Use: Inline system notice ("분류 없이 저장됨")
...
- Padding: 2px 6px
- Font: 11px / 700
- Use: Unseen/reminder count on a project or list (caps to "99+" past 99)
...
- Padding: 2px 6px
- Font: 11px / 500
- Use: Metadata tag (개인/공유 상태, project label)
